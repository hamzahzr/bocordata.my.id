...
if($response===false){log_activity('Pemeriksaan gagal',$user,'Gagal menghubungi layanan pemeriksaan');json_response(['error'=>'Gagal menghubungi API','detail'=>$curlError],502);}
$data=json_decode($response,true);
if($data===null&&json_last_error()!==JSON_ERROR_NONE){log_activity('Pemeriksaan gagal',$user,'Respons bukan JSON');json_response(['error'=>'API mengembalikan respons yang bukan JSON.'],502);}
log_activity('Pemeriksaan data',$user,'Identifier: '.substr($query,0,3).'***');
